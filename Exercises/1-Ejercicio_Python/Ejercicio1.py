...
def extraer_lista(file):
    f = open (file, "r",encoding='utf-8') #Abre el fichero en modo lectura con codificación utf-8
    s = f.read() #Lee el contenido del fichero
    '''
    La función re.findall busca todas las coincidencias de una expresión regular en una cadena 
    y devuelve una lista con todas las coincidencias.
    r'' indica que es una cadena de texto en bruto, es decir, que los caracteres especiales no se interpretan.
    <title>(.*)</title> busca todas las cadenas que empiecen por <title> y terminen por </title> y que tengan cualquier cosa en medio.
    \s* busca cualquier número de espacios en blanco.
    '''
    l1 = re.findall(r'<title>(.*)</title>\s*<link>(.*)</link>', s) 
    l2 = re.findall(r'<pubDate>(.*)</pubDate>', s) 
    l = [list(e1) for e1 in l1[1:]] #Crea una lista con las noticiasm
